  ...
  ask mantas [
    ...
    ifelse any? mantamates[
      ...
      ifelse distance nearest-manta < manta-separation [ ;; If the manta is swimming too close to the other mantas it will turn away from them
        let desired-heading  weighted-direction  heading calculate-heading  average-manta-heading flow-heading impact-of-plankton  impact-of-other-mantas  impact-of-flow
      ...
      let desired-heading  weighted-direction  heading calculate-heading  0 flow-heading impact-of-plankton  0 impact-of-flow
      let desired-turn subtract-headings desired-heading heading
...
to-report weighted-direction [initialD planktonD mantasD waterD planktonWeight  mantasWeight  flowWeight]
  let initialWeight 1
  ...
  let x3 (initialWeight  * cos(initialD) + planktonWeight  * cos(planktonD) + mantasWeight  * cos(mantasD) + flowWeight * cos(waterD))
  let y3 (initialWeight  * sin(initialD) + planktonWeight  * sin(planktonD) + mantasWeight  * sin(mantasD) + flowWeight  * sin(waterD))
  let angle3 atan y3 x3
  set angle3 angle3 * 180 / pi
  report angle3
end
@#$#@#$#@
GRAPHICS-WINDOW
